...
with st.sidebar:
    ...
    with st.expander("A) Capacitate & Geometrie", expanded=True):
        ...
        L_over_D = st.number_input(
        "Raport L/D [-] (recomandat 2–5)", min_value=1.50, max_value=6.00, value=3.00, step=0.01, format="%.2f"
        )
        D = st.number_input("Diametru D [m]", value=3.2, min_value=0.1, step=0.05)
